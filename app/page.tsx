...
import {
  StyledMobileAlertContainer,
  StyledMobileAlertText,
} from "$/components/MobileAlert/MobileAlert.styles";
...
export default function Home() {
  ...
  const [isMobile, setIsMobile] = useState<boolean>(size ? size <= 650 : false);
  ...
  useLayoutEffect(() => {
    const context = gsap.context((self) => {
      if (!isMobile) {
        ...
        const firstNumberIndex = self?.selector?.(
          ".number-index:first-of-type"
        );
